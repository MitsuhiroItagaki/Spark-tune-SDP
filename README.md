...
`create_dashboard.py` creates and publishes a Lakeview dashboard with 7 analysis pages.
...
There are two ways to create or update the dashboard. Both automatically detect and update an existing dashboard with the same name (using PATCH), preserving the URL.
...
```bash
# Always unset DATABRICKS_TOKEN first (environment variable interference)
unset DATABRICKS_TOKEN

python3 create_dashboard.py
```

Update `SCHEMA`, `WAREHOUSE_ID`, and `PARENT_PATH` at the top of `create_dashboard.py` as needed.

> **Note:** Both scripts detect an existing dashboard by workspace path (`PARENT_PATH/DASH_NAME.lvdash.json`) and update it in-place via PATCH. The dashboard URL never changes across updates.
...
├── create_dashboard.py              # Lakeview dashboard creation script (local execution)
...
`create_dashboard.py` を実行すると Lakeview ダッシュボード（7ページ）が作成・公開されます。
...
2つの方法があります。いずれも同名ダッシュボードを自動検出して PATCH で上書き更新するため、**URLは変わりません**。

**方法A — Databricks ノートブック（推奨）**
...
**方法B — ローカル Python スクリプト**

```bash
# DATABRICKS_TOKEN 環境変数の干渉を避けるため必ず先に実行
unset DATABRICKS_TOKEN

python3 create_dashboard.py
```

`create_dashboard.py` 冒頭の `SCHEMA`・`WAREHOUSE_ID`・`PARENT_PATH` を環境に合わせて変更してください。

> **Note:** どちらもワークスペースAPIで `PARENT_PATH/DASH_NAME.lvdash.json` を直接検索し、存在すれば PATCH で中身を上書きします。ダッシュボードの URL は更新のたびに変わりません。
...
├── create_dashboard.py              # Lakeview ダッシュボード作成スクリプト（ローカル実行用）
